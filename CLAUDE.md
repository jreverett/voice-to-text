...
- **Groq API** is selected with `Transcription.Engine=Groq`. It downsamples the captured WAV to 16kHz mono FLAC via ffmpeg (Groq caps uploads at 25MB and the raw WAV is the device's native format, often 48kHz stereo float) then sends it to `https://api.groq.com/openai/v1/audio/transcriptions` (OpenAI-compatible). The key is entered in Settings and stored at `%LOCALAPPDATA%\VoiceToText\groq_api_key.txt`, or read from the `GROQ_API_KEY` environment variable as a fallback.
...
- **FLAC compression for Groq uploads**: `CompressForGroq` downsamples the WAV to 16kHz mono FLAC before upload. This is what Whisper resamples to internally, so there's no accuracy loss, and it keeps recordings under Groq's 25MB limit (a native-format WAV hits it after ~60-90s). Falls back to the raw WAV if ffmpeg is missing or the transcode fails.
...
- **Voice commands (LLM intent routing)**: opt-in (`[Commands] Enabled`, default off). When the transcript starts with the wake word (`WakeWord`, default "computer") it is routed to Groq's chat endpoint instead of being pasted. `RunVoiceCommand` builds a live list of open windows (`GetCommandWindows` → title + exe, numbered) and the user's launch aliases (`[CommandAliases]`, e.g. `email=outlook.exe`), then one `chat/completions` POST (JSON mode, `Model` default `llama-3.3-70b-versatile` — 8b misrouted launches) returns a bounded action: `focus` a window *by its number from the list*, `launch` an alias key or a bare runnable token, `tab` (cycle a browser window's tabs via Ctrl+Tab until the active-tab title contains `query`, since tabs aren't separate windows), `keys` an AHK send string, or `none`. Any action can carry a `then` follow-up shortcut sent once the target window is active (`SendThenKeys` — waits for load on launch), so "start drafting an email" launches/focuses Outlook then presses `^+m`. The model only ever picks from sets we supply, and `ExecuteCommandAction` validates before acting (window index in range; launch target is a config alias or matches `^[\w.\-]+$` — a bare token, never a path or arbitrary command string). Win+L in `keys` uses `LockWorkStation` like the send rules. Needs a Groq key even in local Whisper mode (the LLM call is always Groq). Reuses the existing curl + `ParseJsonString`/`JsonUnescape` plumbing (`JsonEscape` added for request bodies; request/response go through `temp\command_request.json`/`command_output.txt`). Settings has a toggle + wake-word field in the Dictation group; the alias map is config-only for now.
