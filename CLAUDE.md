...
- **Send trigger word**: opt-in (`[Send] Enabled`/`Word`, default off/"go"). When the transcript ends with the trigger word it is stripped and `{Enter}` is sent after pasting. Trailing comma-family punctuation (`, ; :`) the model inserts before the word is removed too, but sentence-ending `. ! ?` is preserved. Done in post-processing (deterministic) rather than via the Groq prompt (Whisper prompts only bias, they don't reliably obey removal).
...
- **Config-driven**: all paths, mic device, hotkey, transcription engine, Groq options, threads, and server port live in `config.ini`. The AHK script reads these at startup.
